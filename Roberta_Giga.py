...
cache_dir = ""   # UPDATED path after download
tokenizer = RobertaTokenizer.from_pretrained("roberta-base", cache_dir=cache_dir, local_files_only=True)
...
class RobertaForExtractiveSummarization(nn.Module):
    def __init__(self, pretrained_model_path):
        super().__init__()
        self.roberta = RobertaModel.from_pretrained(pretrained_model_path, local_files_only=True)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        logits = self.classifier(cls_output).squeeze(-1)  # Make shape (batch_size,)
        return logits
...
model = RobertaForExtractiveSummarization(pretrained_model_path=pretrained_model_path).to(device)
...
save_path = os.path.join(save_dir, "best_roberta_gigaword_model.pt")
checkpoint_path = os.path.join(save_dir, "best_roberta_checkpoint.pt")


# === Helper: ROUGE-L Computation ===
