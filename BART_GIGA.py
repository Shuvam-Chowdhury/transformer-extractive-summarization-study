...
cache_dir = ""
tokenizer = BartTokenizer.from_pretrained("facebook/bart-base", cache_dir=cache_dir, local_files_only=True)
...
class BartForExtractiveSummarization(nn.Module):
    def __init__(self, pretrained_model_path):
        super().__init__()
        self.bart = BartModel.from_pretrained(pretrained_model_path, local_files_only=True)
        self.classifier = nn.Linear(self.bart.config.hidden_size, 1)

    def forward(self, input_ids, attention_mask):
        outputs = self.bart(input_ids=input_ids, attention_mask=attention_mask)
        cls_output = outputs.last_hidden_state[:, 0, :]  # Take [CLS]-like token
...
model = BartForExtractiveSummarization(pretrained_model_path=pretrained_model_path).to(device)
...
save_path = os.path.join(save_dir, "best_bart_gigaword_model.pt")
checkpoint_path = os.path.join(save_dir, "best_bart_gigaword_checkpoint.pt")

# Helper for ROUGE-L Computation
...
print("\n All done training and evaluating BART on GIGAWORD!")